...
import type { OtpDeliveryStatus } from "@/types/activation";

interface OtpVerificationFormProps {
  serviceNumber: string;
  maskedPhone: string;
  initialStatus: OtpDeliveryStatus;
}
...
export function OtpVerificationForm({
  ...
  function onSubmit(values: OtpVerificationInput) {
    ...
    startTransition(async () => {
      const result = await submitOtpVerification(values);
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          <FieldGroup>
            <Field data-invalid={otpInvalid ? true : undefined}>
              <FieldLabel htmlFor="otp-0">6-Digit Code</FieldLabel>
              <Controller
                control={control}
                name="otp"
                render={({ field }) => (
